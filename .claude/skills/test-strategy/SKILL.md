---
name: test-strategy
description: Test strategy for THIS project (GHC — multi-tool development harness) — unittest per public function, Markdown test cases, a test report = case copy + result with commit-id/case-version, stdout log link, keep failures with root-cause + countermeasure, commit-before-test, same 3-set for e2e. Tool-specific knobs (module scope, non-determinism seam, e2e scenario, extended version stamps) are fixed per tool via DD#. Use when planning HOW to test the implementation. NOT spec/design (see domain-model/schema-design), NOT asset auditing (see asset-auditor).
...
> 由来・差分は [tailoring-registry](../../tailoring-registry.md)。
