...
| AC-206 | whitelist global enabled=false でスキップ | `create_file` | `{"filePath": "docs/foo.md"}` | whitelist 全体 `enabled=false` + blacklist deny + whitelist 調停=whitelist | whitelist マッチングを行わず deny を返す |
| AC-207 | whitelist write_rules.enabled=false でスキップ | `create_file` | `{"filePath": "docs/foo.md"}` | whitelist write_rules `enabled=false` + blacklist deny + whitelist 調停=whitelist | 操作タイプ単位で whitelist マッチングを行わず deny を返す |
| AC-208 | whitelist rule.enabled=false でスキップ | `create_file` | `{"filePath": "docs/foo.md"}` | whitelist 項目 `enabled=false` + blacklist deny + whitelist 調停=whitelist | 設定項目単位で whitelist マッチングを行わず deny を返す |
...
| AC-200 | whitelist 設定の階層読み込み | global/group/item で enabled と arbitration を設定 | `AccessControlConfig.whitelist` に各階層値が正しく格納される |
